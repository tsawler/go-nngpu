...
#include "metal_bridge.h" // Include  C header
...
int create_gpu_buffer(float *data, long length_bytes, GPUPtr *outGPUPtr,
                      DevicePtr *outDevicePtr, CError *err) {
  @autoreleasepool {
    ...
    if (!_global_mtl_device_ptr) {
      ...
      _global_mtl_device_ptr =
          (__bridge_retained void *)device; // Retain to manage lifetime
      ...
      _global_mtl_command_queue_ptr =
          (__bridge_retained void *)commandQueue; // Retain
    }
    ...
    if (!buffer) {
      set_c_error_message(err,
                          @"Could not create Metal buffer with bytesNoCopy.");
      return -3;
    }

    *outGPUPtr = (__bridge_retained void *)buffer; // Retain the buffer pointer
    *outDevicePtr =
        _global_mtl_device_ptr; // Pass back the device pointer for future ops

    return 0; // Success
  }
}

int retrieve_gpu_buffer_data(GPUPtr gpuPtr, float *data, long length_bytes,
                             CError *err) {
  @autoreleasepool {
...
int perform_mps_matrix_multiplication(GPUPtr aMatrixPtr, long aRows, long aCols,
                                      GPUPtr bMatrixPtr, long bRows, long bCols,
                                      GPUPtr resultMatrixPtr, long resultRows,
                                      long resultCols, DevicePtr mtlDevicePtr,
                                      CError *err) {
  @autoreleasepool {
    id<MTLDevice> device = (__bridge id<MTLDevice>)mtlDevicePtr;
    id<MTLCommandQueue> commandQueue =
        (__bridge id<MTLCommandQueue>)_global_mtl_command_queue_ptr;

    if (!device || !commandQueue) {
      set_c_error_message(err,
                          @"Metal device or command queue not initialized.");
      return -1;
    }
    ...
    MPSMatrix *matrixA = [[MPSMatrix alloc] initWithBuffer:A_buffer
                                                descriptor:descA];
    ...
    MPSMatrixDescriptor *descB =
        [MPSMatrixDescriptor matrixDescriptorWithRows:bRows
                                             ...
                                             dataType:MPSDataTypeFloat32];
    MPSMatrix *matrixB = [[MPSMatrix alloc] initWithBuffer:B_buffer
                                                descriptor:descB];
    ...
    MPSMatrixDescriptor *descC =
        [MPSMatrixDescriptor matrixDescriptorWithRows:resultRows
                                             ...
                                             dataType:MPSDataTypeFloat32];
    MPSMatrix *matrixC = [[MPSMatrix alloc] initWithBuffer:C_buffer
                                                descriptor:descC];
    ...
    if (!matrixMultiplication) {
      set_c_error_message(err,
                          @"Failed to create MPSMatrixMultiplication kernel.");
      return -4;
    }
    ...
    [commandBuffer
        waitUntilCompleted]; // Synchronous for simplicity, can be async

    if (commandBuffer.error) {
      set_c_error_message(err, @"Metal command buffer error: %@",
